...
export default function Webhooks() {
  ...
  useEffect(() => {
    ...
    if (storedApiKey) {
      ...
      fetchStats(storedApiKey);
    }
  }, []);

  const fetchStats = async (key) => {
    try {
      const response = await fetch(`${YOUR_API_URL}/api/user/stats`, {
...
?>`;
  ...
  // Quick Stats
  const quickStats = [
    { label: 'Webhook Status', value: webhookStatus?.registered ? 'Active' : 'Inactive', icon: Radio, color: webhookStatus?.registered ? 'text-emerald-400' : 'text-gray-500' },
    { label: 'Messages Sent', value: stats?.messagesSent || 0, icon: Activity, color: 'text-cyan-400' },
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-[10px] sm:text-xs uppercase font-bold tracking-widest mt-1">
